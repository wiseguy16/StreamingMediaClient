...
class APIController
{
    ...
    var theMonkeys = [MonkeyChannel]()
    var theMonkeyVideos = [MonkeyVideo]()
            ...
            DispatchQueue.main.async(execute: { () -> Void in
                ...
                if let vimeoData = data, let jsonResponse = try? JSONSerialization.jsonObject(with: vimeoData, options: []) as? [String: AnyObject], let myJSON = jsonResponse
                {
                   ...
                   // let dataArray = myJSON["data"] as? [[String: AnyObject]]
                    ...
                    {
                       // print(constDict)
                        let aMonkey = MonkeyChannel(dictionary: constDict)
                        self.theMonkeys.append(aMonkey)
                        self.delegate.gotTheVideos(withOrder: "a", theMonkeys: self.theMonkeys)
...
//*********** FIX HERE **************!!!!
                       ...
                       // self.delegate.gotTheseVideos(withOrder: "b", theMonkeys: self.theMonkeys)
                        // ******************************************** COME BACK AND USE THIS !!!  ********************************
